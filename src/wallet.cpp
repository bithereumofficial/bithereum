...
// Bithereum: optional setting to unlock wallet for block minting only;
...
// Bithereum: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // Bithereum: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // Bithereum: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // Bithereum: not avatar mode
                        {
...
// Bithereum: create coin stake transaction
...
// Bithereum: check 'spent' consistency between wallet and txindex
// Bithereum: fix wallet spent state according to txindex
...
// Bithereum: disable transaction (only for coinstake)
