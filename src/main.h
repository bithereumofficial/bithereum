...
static const int64 MAX_MINT_PROOF_OF_WORK = 70 * COIN;
...
static const int STAKE_MIN_AGE = 60 * 60 * 24 * 2; // minimum age for coin age 2 days
...
public:
    ...
    {
        // Bithereum: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // Bithereum: get transaction coin age
...
public:
    ...
    // Bithereum: block signature - signed by coin base txout[0]'s owner
    ...
    // Bithereum: two types of block: proof-of-work or proof-of-stake
    ...
    // Bithereum: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // Bithereum: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // Bithereum: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // Bithereum: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // Bithereum: block index flags
    enum  
    {
